...
<nav class="navbar mb-8 flex items-center justify-between bg-white px-4 py-3 shadow dark:bg-surface-800">
	...
	<!-- Desktop nav links -->
	<div class="hidden md:flex flex-0 items-center justify-center gap-2">
	...
	<!-- Mobile menu -->
	{#if menuOpen}
		<div class="absolute left-0 top-16 z-50 w-full bg-white shadow-md dark:bg-surface-800 md:hidden animate-fade-in">
			<div class="flex flex-col items-center gap-2 py-4">
				<a href="/bands" class="btn-ghost btn-md btn text-lg w-11/12" on:click={() => (menuOpen = false)}>Bands</a>
				<a href="/profiles" class="btn-ghost btn-md btn text-lg w-11/12" on:click={() => (menuOpen = false)}>Profiles</a>
				{#if data?.session}
					<a
						href='/profile/{data.session.profileId}'
						class="avatar btn-ghost btn-md btn flex h-12 w-11/12 items-center justify-center overflow-hidden rounded-full border-2 border-primary-400 p-0"
						title="Profile"
					...
					<form method="POST" action="/signout" class="inline w-11/12">
						<button class="btn-error btn-md btn text-lg w-full mt-2" type="submit" on:click={() => (menuOpen = false)}>Log Out</button>
					</form>
				{:else}
					<a href="/signup" class="btn-primary btn-md btn text-lg w-11/12" on:click={() => (menuOpen = false)}>Sign Up</a>
					<a href="/signin" class="btn-ghost btn-md btn text-lg w-11/12" on:click={() => (menuOpen = false)}>Sign In</a>
				{/if}
			</div>
		</div>
	{/if}
